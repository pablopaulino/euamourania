# Documentação do Eu Amo Urânia

Esta pasta é a fonte oficial para manutenção do portal.

- [Arquitetura](ARQUITETURA.md)
- [Estrutura de pastas e componentes](ESTRUTURA-E-COMPONENTES.md)
- [Banco de dados e Supabase](BANCO-E-SUPABASE.md)
- [Vercel, APIs e deploy](VERCEL-APIS-DEPLOY.md)
- [Variáveis de ambiente](VARIAVEIS-DE-AMBIENTE.md)
- [Segurança, autenticação e permissões](SEGURANCA.md)
- [Usuários administrativos e matriz de acesso](USUARIOS-E-PERMISSOES.md)
- [Configurações globais e conteúdo administrável](CONFIGURACOES-GLOBAIS.md)
- [CMS e módulos](CMS-E-MODULOS.md)
- [Fluxo editorial e audiência](FLUXO-EDITORIAL-E-AUDIENCIA.md)
- [SEO, performance e acessibilidade](SEO-PERFORMANCE-ACESSIBILIDADE.md)
- [Operação, backup e recuperação](OPERACAO-BACKUP.md)
- [Manual do administrador e editor](MANUAIS.md)
- [Testes e checklist](TESTES-E-CHECKLIST.md)
- [Solução de problemas](TROUBLESHOOTING.md)
- [Roadmap](ROADMAP.md)

Regra de manutenção: toda alteração de tabela, variável, API ou fluxo administrativo deve atualizar o documento correspondente no mesmo pull request.
