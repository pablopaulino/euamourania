...
| `GA_MEASUREMENT_ID` | Vercel/API | pública | ID `G-...` usado após consentimento |
...
Após alterar uma variável, faça novo deploy. Valide login, leitura pública, newsletter de teste, descadastro, consentimento GA4 e a Central de Audiência. Em caso de vazamento, revogue a chave no provedor, gere outra e atualize a Vercel imediatamente.
