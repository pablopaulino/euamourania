    ...
    <tr>
      ...
      <td class="verification-row-actions">
        <div class="verification-row-main-actions">
          ...
          <button type="button" class="verification-more-trigger" data-action-menu="${item.id}" aria-label="Mais ações para ${escapeHtml(item.nome || "atrativo")}" aria-haspopup="menu" aria-expanded="false">•••</button>
        </div>
      </td>
    </tr>`;
}
...
function openActionPopover(trigger, item) {
  ...
  popover.setAttribute("aria-label", `Mais ações para ${item.nome || "atrativo"}`);
