...
const linksIniciais = [
  ["Nosso site", "https://www.euamourania.site/", "🌐"],
  ["Canal WhatsApp", "https://whatsapp.com/channel/0029VapPdlLGpLHTXQELk210", "💬"],
  ["WhatsApp", "https://wa.me/5517981344558", "📱"],
  ["YouTube", "https://www.youtube.com/@EuAmoUr%C3%A2nia", "▶️"],
  ["Instagram", "https://instagram.com/euamourania", "📷"],
  ["Facebook", "https://facebook.com/euamourania", "👥"],
  ["X (Twitter)", "https://x.com/euamourania", "𝕏"],
  ["TikTok", "https://tiktok.com/@euamourania", "🎵"]
].map(([titulo, url, icone], ordem) => ({ titulo, url, icone, ordem, status: "ativo" }));
...
async function migrar() {
  ...
  status.textContent = "Lendo os dados e preparando a importação…";
  const supabase = getSupabase();
  try {
    const [noticiasAntigas, guiaAntigo] = await Promise.all([
      lerJson("../news-data.json"),
      lerJson("../guia-data.json")
    ]);
    ...
    const guia = guiaAntigo.map(item => ({
      ...
      seo_descricao: textoPuro(item.descricao || "").slice(0, 160)
    }));

    const turismo = guiaAntigo.filter(item => item.categoria === "turismo").map(item => ({
      nome: item.nome,
      slug: gerarSlug(item.nome),
      descricao: item.descricao || "",
      conteudo_html: item.descricao ? `<p>${item.descricao}</p>` : "",
      imagem_url: item.imagem || null,
      endereco: item.endereco || null,
      whatsapp: item.whatsapp || null,
      status: "publicado",
      destaque: Boolean(item.destaque),
      seo_titulo: `${item.nome} | Turismo em Urânia`,
      seo_descricao: textoPuro(item.descricao || "").slice(0, 160)
    }));

    const categorias = [
      { nome: "Notícias", slug: "noticias", tipo: "noticias", ordem: 0, status: "ativo" },
      ...[...new Set(guiaAntigo.map(item => item.categoria).filter(Boolean))].map((nome, ordem) => ({ nome, slug: gerarSlug(nome), tipo: "guia", ordem, status: "ativo" })),
      { nome: "Pontos turísticos", slug: "pontos-turisticos", tipo: "turismo", ordem: 0, status: "ativo" }
    ];
    ...
    status.innerHTML = `<strong>Migração concluída.</strong><br>${noticias.length} notícias, ${guia.length} itens do guia, ${turismo.length} ponto turístico e ${linksIniciais.length} links conferidos.`;
...
if (acesso?.configurado) {
  document.getElementById("admin-name").textContent = acesso.admin.nome || acesso.user.email;
  button.addEventListener("click", migrar);
  status.textContent = "Pronto para importar os arquivos antigos. A operação é segura para repetir.";
}
