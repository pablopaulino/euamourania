...
const normalizeGalleryUrls = value => Array.isArray(value) ?value.map(item => typeof item === "string" ?item : item?.url || item?.imagem_url).filter(Boolean).slice(0,5) : [];
function galleryUrlsHtml(name,label,value){
  ...
  return `<fieldset class="full-row cms-gallery-field" data-gallery-field="${name}"><legend>${label}</legend><p class="field-hint">Adicione até 5 fotos extras para o carrossel do aplicativo. A imagem principal continua no campo acima.</p><div class="cms-gallery-list">${Array.from({length:5},(_,index)=>`<label>Foto ${index+1}<input type="text" inputmode="url" data-type="url" data-cms-image="true" data-media-folder="${escapeHtml(folder)}" data-media-preset="card" name="${name}_${index}" value="${escapeHtml(urls[index]||"")}" placeholder="https://... ou /assets/..."></label>`).join("")}</div></fieldset>`;
}
function collectGalleryUrls(form,name){
  return Array.from({length:5},(_,index)=>String(form.get(`${name}_${index}`)||"").trim()).filter(Boolean).filter((url,index,array)=>array.indexOf(url)===index);
}
