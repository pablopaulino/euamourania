...
function buttonForNav([key,label,module],isIndex,current){
  ...
  if(isIndex){
    if(["dashboard","noticias","colaboradores_voluntarios","guia_comercial","turismo","links","eventos","eventos_principais","eventos_edicoes","categorias","configuracoes_site","comunicacao","notificacoes"].includes(key))attrs.push(`data-view="${key}"`);
...
function normalizeAdminNavigation(){
  ...
  const shellViews=["dashboard","noticias","aprovacoes","colaboradores_voluntarios","guia_comercial","turismo","links","eventos","eventos_principais","eventos_edicoes","categorias","audiencia","insights","configuracoes_site","midia","banners","comunicacao","notificacoes"];
