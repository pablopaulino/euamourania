export const OFFICIAL_REGULAMENTO_2026 = `Regulamento Oficial
...
export const OFFICIAL_METODOLOGIA_2026 = `Metodologia de Apuração
...
export function getDefaultRegulamento(ano) {
  return Number(ano) === 2026 ? OFFICIAL_REGULAMENTO_2026 : "";
}

export function getDefaultMetodologia(ano) {
  return Number(ano) === 2026 ? OFFICIAL_METODOLOGIA_2026 : "";
}

export function renderOfficialDocument(value, fallback = "") {
  const source = String(value || fallback || "").trim();
  if (!source) return "<p>Conteúdo ainda não publicado para esta edição.</p>";
  ...
  const flushList = () => {
    ...
    html.push(`<ul>${list.map(item => `<li>${escapeDocumentHtml(item.replace(/[;.]$/, ""))}</li>`).join("")}</ul>`);
    list = [];
  };
  ...
  lines.forEach((line, index) => {
    if (index === 0) {
      flushList();
      html.push(`<h2>${escapeDocumentHtml(line)}</h2>`);
      return;
    }
    if (/^\d+\.\s+/.test(line)) {
      flushList();
      html.push(`<h3>${escapeDocumentHtml(line)}</h3>`);
      return;
    }
    if (/^[A-ZÁÉÍÓÚÂÊÔÃÕÇ][^.!?]{2,80}:?$/.test(line) && index < 4) {
      flushList();
      html.push(`<p class="awards-document-lead">${escapeDocumentHtml(line)}</p>`);
      return;
    }
    if (/[;]$/.test(line) || /^(Portal oficial|Enquetes oficiais|Troféu oficial|Certificado|Selo digital|Divulgação especial|Instagram:|Portal:|controle|identificação|validações|registro|auditoria|existência|enquadramento|relevância|cumprimento|revisão|integridade|votos duplicados|atividades suspeitas|inconsistências)/i.test(line)) {
      list.push(line);
      return;
    }
    flushList();
    html.push(`<p>${escapeDocumentHtml(line)}</p>`);
  });
  flushList();
  return html.join("");
}
