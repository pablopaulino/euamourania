...
async function carregarNoticia(){
  ...
  try{
    ...
    container.innerHTML=`<article class="news-detail-container"><p class="eyebrow">${esc(noticia.categoria_nome||"Notícias de Urânia")}</p><h1>${esc(noticia.titulo)}</h1>${noticia.subtitulo?`<p class="article-subtitle">${esc(noticia.subtitulo)}</p>`:""}<p class="meta">${esc(noticia.autor)} · ${esc(formatarData(noticia.publicado_em))}</p>${safeImage(noticia.imagem_url)?`<figure><img src="${safeImage(noticia.imagem_url)}" alt="${esc(noticia.legenda_imagem||noticia.titulo)}" class="main-image">${noticia.legenda_imagem?`<figcaption>${esc(noticia.legenda_imagem)}</figcaption>`:""}</figure>`:""}<div class="article-copy">${conteudo}</div><div class="share-buttons"><p>Compartilhe esta notícia</p><a class="btn-share" target="_blank" rel="noopener" href="https://api.whatsapp.com/send?text=${encodeURIComponent(noticia.titulo+" - "+canonical)}">${icons.whatsapp}WhatsApp</a><a class="btn-share" target="_blank" rel="noopener" href="https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(canonical)}">${icons.facebook}Facebook</a><button class="btn-share" id="copy-link" type="button">${icons.copy}Copiar link</button></div><section class="related-news" id="related-news" aria-label="Notícias relacionadas"></section></article>`;
    document.getElementById("copy-link").addEventListener("click",async e=>{await navigator.clipboard.writeText(canonical);e.currentTarget.textContent="Link copiado"});
    const relacionadas=await listarNoticiasRelacionadas(noticia.categoria_nome,noticia.slug);
    if(relacionadas.length)document.getElementById("related-news").innerHTML=`<h2>Continue lendo</h2><div class="related-grid">${relacionadas.map(item=>`<a class="related-card" href="${urlNoticia(item.slug)}">${safeImage(item.imagem_url)?`<img src="${safeImage(item.imagem_url)}" alt="${esc(item.titulo)}" loading="lazy">`:'<div class="related-placeholder">Eu Amo Urânia</div>'}<div class="related-card-body"><p class="related-meta">${esc(item.categoria_nome||"Notícias")} · ${esc(formatarData(item.publicado_em))}</p><h3>${esc(item.titulo)}</h3><p>${esc(resumo(item).slice(0,105))}${resumo(item).length>105?"…":""}</p><span class="read-more">Ler notícia →</span></div></a>`).join("")}</div>`;
