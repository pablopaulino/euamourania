...
async function carregarNoticia() {
  ...
  try {
    ...
    const conteudoSeguro = window.DOMPurify ? window.DOMPurify.sanitize(noticia.conteudo_html, { ADD_TAGS: ["iframe"], ADD_ATTR: ["allow", "allowfullscreen", "frameborder"] }) : noticia.conteudo_html;
    container.innerHTML = `<article class="news-detail-container">
      <p class="eyebrow">${noticia.categoria_nome || 'Notícias de Urânia'}</p>
      <h1>${noticia.titulo}</h1>
      ${noticia.subtitulo ? `<p class="article-subtitle">${noticia.subtitulo}</p>` : ''}
      <p class="meta">${noticia.autor} · ${formatarData(noticia.publicado_em)}</p>
      ${noticia.imagem_url ? `<figure><img src="${noticia.imagem_url}" alt="${noticia.legenda_imagem || noticia.titulo}" class="main-image">${noticia.legenda_imagem ? `<figcaption>${noticia.legenda_imagem}</figcaption>` : ''}</figure>` : ''}
    ...
    if (relacionadas.length) document.getElementById("related-news").innerHTML = `<h2>Continue lendo</h2><div class="related-grid">${relacionadas.map(item => `<a href="?slug=${encodeURIComponent(item.slug)}"><strong>${item.titulo}</strong><span>Ler notícia →</span></a>`).join("")}</div>`;
  } catch (error) {
    console.error(error);
