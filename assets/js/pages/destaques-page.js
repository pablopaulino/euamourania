...
function addStyle() {
  ...
  style.textContent = `
    ...
    .home-editorial-grid{display:grid;grid-template-columns:minmax(0,1.16fr) minmax(320px,.84fr);gap:1.15rem;align-items:stretch}
    .home-news-main{display:grid;grid-template-columns:1fr;grid-template-rows:repeat(2,minmax(0,1fr));gap:1.15rem;min-height:100%}
    ...
    .home-news-lead{display:grid;grid-template-columns:minmax(210px,.42fr) minmax(0,1fr);min-height:0}
    ...
    .home-news-lead-media{min-height:220px}
    .home-news-lead-media img{display:block;width:100%;height:100%;object-fit:cover;transition:transform .3s ease}
    .home-news-lead:hover img{transform:scale(1.03)}
    .home-news-lead-copy{display:flex;flex:1;flex-direction:column;padding:clamp(1.1rem,2.4vw,1.65rem)}
    ...
    .home-news-lead h3{margin:.1rem 0 .7rem;font-size:clamp(1.25rem,2vw,1.58rem);line-height:1.12}
    ...
    .home-news-side{display:grid;gap:1rem;height:100%;padding:1rem}
    ...
    @media(max-width:1100px){.home-editorial-grid{grid-template-columns:1fr}.home-news-main{grid-template-columns:repeat(2,minmax(0,1fr));grid-template-rows:none}.home-news-lead{display:flex;flex-direction:column}.home-news-lead-media{min-height:0;aspect-ratio:16/9}.home-news-side{grid-template-columns:1fr 1fr}.home-popular{padding-top:0;border-top:0}}
    @media(max-width:980px){.home-news-main{grid-template-columns:1fr}.home-news-lead{display:grid;grid-template-columns:minmax(0,.9fr) minmax(260px,1.1fr)}.home-news-lead-media{aspect-ratio:auto}.home-news-side{grid-template-columns:1fr 1fr}.home-popular{padding-top:0;border-top:0}}
    @media(max-width:700px){.home-editorial{padding:2.45rem 0}.home-editorial-head{display:grid;margin-bottom:1rem}.home-editorial-head h2{font-size:1.85rem}.home-editorial-head p:not(.eyebrow){font-size:.92rem}.home-editorial-link{justify-self:start}.home-news-main{gap:.9rem}.home-news-main .home-news-lead:nth-child(n+2){display:none}.home-news-lead,.home-news-side{border-radius:20px}.home-news-lead{display:flex;flex-direction:column}.home-news-lead-media{width:100%;min-height:0;aspect-ratio:16/9}.home-news-lead-media img{width:100%!important;height:100%!important;object-fit:cover!important}.home-news-lead-copy{padding:1rem}.home-news-label{margin-bottom:.6rem;font-size:.62rem}.home-news-lead h3{font-size:1.35rem}.home-news-lead-copy>p:not(.eyebrow){display:-webkit-box;overflow:hidden;font-size:.9rem;line-height:1.48;-webkit-line-clamp:2;-webkit-box-orient:vertical}.home-news-action{padding-top:.75rem;font-size:.82rem}.home-news-side{grid-template-columns:1fr;padding:.75rem}.home-news-latest-card{grid-template-columns:82px minmax(0,1fr)}.home-news-latest-card img,.home-news-placeholder{width:82px;height:70px}.home-popular{padding-top:.8rem;border-top:1px solid var(--line)}}
  `;
...
function render(news = []) {
  ...
  const leads = [lead, ...news.filter((item) => item.id !== lead.id)].slice(0, 2);
  const leadIds = new Set(leads.map((item) => item.id));
  const latest = news.filter((item) => !leadIds.has(item.id)).slice(0, 4);
  const popular = [...news]
    .filter((item) => Number(item.visualizacoes || 0) > 0)
    .sort((a, b) => Number(b.visualizacoes || 0) - Number(a.visualizacoes || 0))
    .slice(0, 5);
  const popularList = popular.length ? popular : news.filter((item) => !leadIds.has(item.id)).slice(0, 5);
  ...
  target.insertAdjacentHTML("afterend", `<section class="home-editorial" aria-labelledby="home-editorial-title">
    <div class="container">
      ...
      <div class="home-editorial-grid">
        <div class="home-news-main">${leads.map(leadCard).join("")}</div>
