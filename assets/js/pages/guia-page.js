...
function renderizar(dados){
 ...
 container.innerHTML=visiveis.map(item=>`<article class="card-guia" id="guia-${esc(item.id)}" data-guide-id="${item.id}">${item.recomendado?'<span class="badge-destaque">Recomendado</span>':""}<div class="card-media">${safeImage(item.imagem_url)?`<img src="${safeImage(item.imagem_url)}" class="card-img-top" alt="${esc(item.nome)}" loading="lazy">`:placeholder}</div><div class="card-body">${item.categoria_nome?`<p class="guide-category">${esc(item.categoria_nome)}</p>`:""}<h2 class="card-title">${esc(item.nome)}</h2><p class="card-text">${esc(item.descricao)}</p><div class="guide-info">${item.endereco?`<p><small>Endereço</small>${esc(item.endereco)}</p>`:""}${item.horario?`<p><small>Horário</small>${esc(item.horario)}</p>`:""}</div>${item.whatsapp?`<a href="https://wa.me/${String(item.whatsapp).replace(/\D/g,"")}" target="_blank" rel="noopener" class="btn-whatsapp">Chamar no WhatsApp</a>`:""}</div></article>`).join("");
