    ...
    <article class="event-public-shell">
      ...
      <div class="container event-public-content">
        ...
        <section class="event-section">
          <div class="event-section-title"><p class="eyebrow">Sobre o evento</p><h2>História e importância</h2></div>
          <div class="event-copy">${evento.historia_html || `<p>${esc(evento.descricao_curta || "As informações históricas deste evento serão atualizadas em breve.")}</p>`}</div>
        </section>
        ${proxima ? `<section class="event-next-edition">
          <div><p class="eyebrow">Próxima edição</p><h2>${esc(proxima.titulo)}</h2><p>${esc(proxima.subtitulo || `${fmtDate(proxima.data_inicio)}${proxima.local ? ` · ${proxima.local}` : ""}`)}</p></div>
        ...
        <section id="edicoes" class="event-section">
          <div class="event-section-title"><p class="eyebrow">Histórico</p><h2>Edições do evento</h2></div>
