...
async function renderIniciativa(req, res, slug) {
  ...
  const contactLinks = [initiative.whatsapp && `<a href="https://wa.me/${esc(String(initiative.whatsapp).replace(/\D/g,''))}" target="_blank" rel="noopener">WhatsApp</a>`, initiative.telefone && `<a href="tel:${esc(String(initiative.telefone).replace(/\D/g,''))}">Telefone</a>`, initiative.email && `<a href="mailto:${esc(initiative.email)}">E-mail</a>`, initiative.site_url && `<a href="${esc(safeHref(initiative.site_url))}" target="_blank" rel="noopener">Site</a>`, initiative.instagram && `<a href="${esc(safeHref(initiative.instagram.startsWith('http') ? initiative.instagram : `https://instagram.com/${initiative.instagram.replace(/^@/,'')}`))}" target="_blank" rel="noopener">Instagram</a>`].filter(Boolean).join(' · ');
  const helpHtml = help.map(entry => `<article class="card"><h3>${esc(entry.titulo)}</h3>${entry.descricao ? `<p>${esc(entry.descricao)}</p>` : ''}${entry.tipo === 'pix' ? `<p><strong>Recebedor: ${esc(entry.recebedor_nome)}</strong></p><button type="button" data-pix="${esc(entry.valor_publico)}">Copiar chave Pix</button>` : entry.valor_publico ? `<a href="${esc(entry.tipo === 'telefone' ? `tel:${String(entry.valor_publico).replace(/\D/g,'')}` : entry.tipo === 'whatsapp' ? `https://wa.me/${String(entry.valor_publico).replace(/\D/g,'')}` : safeHref(entry.valor_publico))}" target="_blank" rel="noopener">Acessar</a>` : ''}</article>`).join('');
  const actionsHtml = actions.map(action => `<article class="card"><h3><a href="/iniciativas/${encodeURIComponent(action.slug)}">${esc(action.titulo)}</a></h3><p>${esc(action.resumo || '')}</p></article>`).join('');
  res.setHeader('Content-Type', 'text/html; charset=utf-8');
  res.setHeader('Cache-Control', 'public, s-maxage=300, stale-while-revalidate=86400');
  return res.status(200).send(`<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${esc(title)}</title><meta name="description" content="${esc(description)}"><link rel="canonical" href="${esc(canonical)}"><meta property="og:type" content="website"><meta property="og:locale" content="pt_BR"><meta property="og:site_name" content="${esc(siteName)}"><meta property="og:title" content="${esc(title)}"><meta property="og:description" content="${esc(description)}"><meta property="og:image" content="${esc(image)}"><meta property="og:url" content="${esc(canonical)}"><meta name="twitter:card" content="summary_large_image"><meta name="twitter:title" content="${esc(title)}"><meta name="twitter:description" content="${esc(description)}"><meta name="twitter:image" content="${esc(image)}"><link rel="stylesheet" href="/styles.css"></head><body><main class="container"><a href="/iniciativas/">← Iniciativas da Comunidade</a><img src="${esc(image)}" alt="" style="width:100%;max-height:420px;object-fit:cover;border-radius:24px;margin-top:24px"><p>Comunidade</p><h1>${esc(initiative.titulo)}</h1><p>${esc(initiative.resumo || '')}</p><h2>Sobre</h2><p>${esc(initiative.descricao || '')}</p>${contactLinks ? `<section><h2>Contatos e redes</h2><p>${contactLinks}</p></section>` : ''}${helpHtml ? `<section><h2>Como ajudar</h2>${helpHtml}<p><small>O Eu Amo Urânia apenas divulga esta iniciativa e não processa transferências. Confira os dados antes de pagar.</small></p></section>` : ''}${actionsHtml ? `<section><h2>Ações desta iniciativa</h2>${actionsHtml}</section>` : ''}</main><script>document.addEventListener('click',function(e){var b=e.target.closest('[data-pix]');if(b)navigator.clipboard.writeText(b.dataset.pix).then(function(){b.textContent='Chave Pix copiada';});});</script></body></html>`);
}
