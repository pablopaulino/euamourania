...
async function renderIniciativa(req, res, slug) {
  ...
  const [helpResponse, actionsResponse] = await Promise.all([
    ...
    fetch(`${SUPABASE_URL}/rest/v1/iniciativas_comunitarias?select=id,titulo,slug,resumo&iniciativa_pai_id=eq.${encodeURIComponent(initiative.id)}&status=eq.publicado&order=atualizado_em.desc`, { headers: { apikey: SUPABASE_KEY } })
  ]);
