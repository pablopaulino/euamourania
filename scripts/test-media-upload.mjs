...
for(const feature of["grid-template-rows:auto auto minmax(0,1fr) auto","overflow-y:auto","display:flex!important","flex-wrap:wrap!important","flex:0 1 calc(25% - .75rem)!important",".cms-library-picker-card>img","height:150px!important",".cms-media-button{display:inline-flex;flex:0 1 auto",".cms-media-input-row .cms-media-actions{grid-column:1;grid-row:2",".cms-media-input-row .cms-media-preview{grid-column:2;grid-row:1/4"])must(styles.includes(feature),`Layout da biblioteca incompleto: ${feature}`);
...
must(admin.includes('querySelectorAll(".admin-nav button,.admin-nav a")')&&admin.includes('classList.toggle("active"'),"Menu administrativo pode manter duas seções ativas");
must(admin.includes('inputType=type==="url"||type==="number"?"text":type')&&admin.includes("validSiteReference(value)"),"Formulários do CMS não aceitam caminhos internos de assets");
