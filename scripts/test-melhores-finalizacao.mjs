...
must(voteApi.includes("TURNSTILE_SECRET_KEY") && voteApi.includes("verifyTurnstile"), "API de voto deve suportar Turnstile/Captcha opcional.");
must(indicationApi.includes("TURNSTILE_SECRET_KEY") && indicationApi.includes("verifyTurnstile"), "API de indicação deve suportar Turnstile/Captcha opcional.");
...
must(editionJs.includes("getTurnstileToken") && editionJs.includes("turnstile_token"), "Frontend deve enviar token Turnstile quando configurado.");
...
console.log("Finalização do Melhores de Urânia validada: Captcha opcional, cron, anti-spam, pós-voto e analytics avançado.");
